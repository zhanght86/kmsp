 ...
 * 
 */
package com.yonyou.kms.common.service;
...
public abstract class BaseService {
	...
	public static final String CATEGORY_PLACE_USER="user";
	public static final String CATEGORY_PLACE_SYS="sys";
	/**
	 * 数据范围过滤
...
//							}
	...
	//add by luqibao 前端页面的组织结构  显示的是当前所属机构以及以上的树结构
	public static String dataScopeFilter1(User user, String officeAlias, String userAlias) {
				...
				for (String oa : org.apache.commons.lang3.StringUtils.split(officeAlias, ",")){
					if (!dataScope.contains(r.getDataScope()) && org.apache.commons.lang3.StringUtils.isNotBlank(oa)){
						if (Role.DATA_SCOPE_ALL.equals(r.getDataScope())){
							...
							sqlString.append(" OR EXISTS (SELECT 1 FROM sys_role_office WHERE role_id = '" + r.getId() + "'");
							sqlString.append(" AND office_id = " + oa +".id)");
						}else if(Role.DATA_SCOPE_OFFICE_ABOVE.equals(r.getDataScope())){
							Office office=user.getOffice();
							String id=office.getId();
							String ids[]=office.getParentIds().replace("0,", "").split(",");
							...
							sqlString.append(" OR "+oa+".id in ("+sql.toString()+")");
						}else if(Role.DATA_SCOPE_COMPANY_ABOVE.equals(r.getDataScope())){
						}
						//else if (Role.DATA_SCOPE_SELF.equals(r.getDataScope())){
						dataScope.add(r.getDataScope());
					}
