...
import com.yonyou.kms.common.web.Servlets;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		// 校验用户名密码
		UserSchema schema = SchemaUtils.findUserSchemaByLoginName(token.getUsername());
		if(schema==null){
			return null;
		}
		//SchemaContextHolder.setSchemaype(schema.getTenantSchemaName());
		UserUtils.putCache("schema", schema.getTenantSchemaName());
		UserUtils.putCache("schema-private", schema.getTenantSchemaName());
		CacheUtils.put(UserUtils.USER_CACHE, "schema", schema.getTenantSchemaName());
