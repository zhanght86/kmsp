	...
	<script type="text/javascript" >
		function selectAll(){
        	...
        	if(selectall.checked){
        		var select=document.getElementsByName("selectrole");
		...
		$(document).ready(function() {
			var st1=$("input[name='selectrole']:checked");
     		var st2=$("input[name='selectrole']");
			...
			<tbody>
				<c:forEach items="${roles}" var="roles">
					<tr>
						<td>${roles.name}</td>
						<td>
							<c:if test="${roles.isChecked=='0'}">
							选择<input name="selectrole" type="checkbox"  value="${roles.id}" role="${roles.name}"/>
							</c:if>
							<c:if test="${roles.isChecked=='1'}">
							选择<input name="selectrole" type="checkbox" checked="checked" value="${roles.id}" role="${roles.name}"/>
							</c:if>
						</td>
					</tr>
				</c:forEach>
