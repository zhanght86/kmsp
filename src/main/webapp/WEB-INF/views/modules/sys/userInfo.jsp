...
<head>
	...
	$(document).ready(function() {
		$("#photo").click(function(){
			 $.jBox("iframe:${ctx}/sys/user/userPhoto",{  
          		 	title: "操作提示：拖动鼠标选择区域; 双击保存",  
            		width: 600,  
            		height: 400,
            		buttons: { '关闭': false}
        	});
...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class=""><a href="${ctx}/sys/user/modifyPwd">修改密码</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/info" method="post" class="form-horizontal"><%--
		...
		<div class="control-group">
			...
			<input type="hidden" name="photo" value="${user.photo}">
 			<c:choose>
    			<c:when test="${user.photo=='1'}">
    				<img id="photo" src="${ctxStatic}/images/kms_userPhoto.png" width="100px" height="100px"/>
    			</c:when>
    			<c:otherwise>
        			<img id="photo" src="${user.photo}" width="100px" height="100px"/>
    			</c:otherwise>
			</c:choose>
			</div>
		</div>
